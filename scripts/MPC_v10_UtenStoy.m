%% ============================================================
%  ECONOMIC MPC v10 — 3-INPUT PROSESSMODELL — 2-LANE (UTEN STØY)
%  - VERSJON UTEN AR(1) STØY 
%  ============================================================

clear; clc; close all;
fprintf('=========================================================\n');
fprintf('  ECONOMIC MPC — CURLINGHALL 2-LANE (3-input prosessmodell)\n');
fprintf('  UTEN AR(1) STØY — for sammenligning med støy-variant\n');
fprintf('==============================================================\n\n');
...
%% ============================================================
%  SEKSJON 8: PRECOMPUTE KONSTANTE MATRISER
%% ============================================================
...
%% ============================================================
%  SEKSJON 9: MPC SIMULERING (uten støy)
...
% Konverter til tilstandsrom for prosessmodell-simulator:
...
% Halltemp (DV) i normalisert form — brukes både i prosessmodell-sim og prediksjon:
hallTemp_norm = u_sim_norm(:, 2);

% Warm-start prosessmodell-simulatoren med 3 inputs:
x_plant = zeros(nx, 1);
for kw = 1:N_warmup
...
% Initial output med halltemp som DV (uten støy):
u_init = [u1_mpc_n(k_start-1); hallTemp_norm(k_start-1); u2_mpc_n(k_start-1)];
y0 = C_plant * x_plant + D_plant * u_init;
...
for k = k_start:(N - Np - 1)
    ...
    if any(~isfinite(f_vec)) || any(~isfinite(b_upper))
        ...
        y_mpc_n(k) = C_plant * x_plant + D_plant * u_plant_k;  % uten støy
        nFail = nFail + 1;
        continue;
    end
    ...
    % Prosessmodell-simulator med 3 inputs (ventil, halltemp, pumpe) — uten støy:
...
%% ============================================================
%  SEKSJON 10: HISTORISK BASELINE (uten støy)
%  Også med 3 inputs (ventil, halltemp, pumpe).
%% ============================================================
fprintf('=== SEKSJON 11: Historisk baseline (uten støy) ===\n');
...
y_base_n(k_start-1) = C_plant * x_base + D_plant * u_b_init;  % uten støy

for k = k_start:N
    u_b = [u_sim_norm(k,1); u_sim_norm(k,2); u_sim_norm(k,3)];
    x_base = A_plant * x_base + B_plant * u_b;
    y_base_n(k) = C_plant * x_base + D_plant * u_b;  % uten støy
...
fprintf('\n==============================================================\n');
fprintf('RESULTATER — Economic MPC — 2-lane (3-input prosessmodell, UTEN STØY)\n');
fprintf('==============================================================\n');
...
fprintf('Prosessmodell-sim: 3-input håndtunet TF UTEN AR(1) støy\n\n');

% Lag separat gruppemerking for å rense idle-tallene fra forkjøling:
...
figure('Name','Istemperatur 2-lane (uten støy)','Position',[50 50 1400 550]);
hold on;
for k = 1:N-1
...
title(sprintf(['MPC 2-lane (3-input) UTEN STØY | Cost: MPC=%.0f, Hist=%.0f NOK (%+.1f%%)\n' ...
...
% --- Figur 2: Ventil + pris ---
figure('Name','MV1 + Pris (uten støy)','Position',[50 50 1400 550]);
...
% --- Figur 3: Pumpe ---
figure('Name','MV2 (uten støy)','Position',[50 50 1400 380]);
hold on;
...
title('MV2: Pumpe (uten støy)'); legend('Location','best'); grid on; xlim([0 t_end]);

% --- Figur 4: Kumulativ kostnad ---
figure('Name','Kumulativ kostnad (uten støy)','Position',[50 50 1400 380]);
hold on;
...
title(sprintf('Kumulativ Energikostnad UTEN STØY (%+.1f%%)', savings_pct));
...
figure('Name','Zoom (uten støy)','Position',[50 50 1400 700]);
...
ylabel('°C'); title('Zoom: Istemperatur 2-lane (uten støy)'); grid on;
...
sgtitle('3-dagers zoom — MPC 2-lane (uten støy)');

% --- Figur 7: Gruppespesifikk KPI-oversikt ---
figure('Name','Gruppe-KPI (uten støy)','Position',[50 50 1000 550]);
...
sgtitle(sprintf('KPI-er — MPC 2-lane UTEN STØY | Besparelse: %.1f NOK (%.1f%%)', ...
    savings, savings_pct));
