%% ============================================================
%  ECONOMIC MPC — 3-INPUT PROSESSMODELL med støy — 2-lane
%  ============================================================

clear; clc; close all;
fprintf('=========================================================\n');
fprintf('  ECONOMIC MPC — CURLINGHALL 2-LANE (3-input prosessmodell)\n');
fprintf('==============================================================\n\n');
...
modelFile     = fullfile(dataDir, 'plant_model_lane2_3input.mat');
modelDataFile = fullfile(dataDir, 'MergedData_NEW.csv');
bookingFile   = fullfile(dataDir, '2026_simulerte_bookinger.csv');
priceFile     = fullfile(dataDir, 'strompris_lane2_2026.csv');
...
% Warm-start:
N_warmup = 12; %2 timer warm-start for stabilisering

%% ============================================================
%  SEKSJON 2: LAST PROSESSMODELL OG DATA
%  Prosessmodellen og normaliseringsparametrene lastes fra .mat-fil 
%  (laget i prosessmodell-skriptet). 3 inputs: ventil (MV1), 
%  halltemp (DV), pumpe (MV2).
%% ============================================================
fprintf('=== SEKSJON 2: Laster prosessmodell og data ===\n');

% Last prosessmodell og normaliseringsparametre fra fil:
...
% --- Hent ut signaler ---
% 3 inputs: ventil, halltemp, pumpe (samme rekkefølge som prosessmodellen)
...
%% ============================================================
%  SEKSJON 3: ENERGIMODELL 
%% ============================================================
fprintf('=== SEKSJON 3: Energimodell fra data ===\n');

% Bruker kompressor- og viftesignaler som energiproxy siden direkte energimålinger 
...
dv_glycol_dT = V2.glycol_in - V2.glycol_out;   % temperaturdifferanse glycolkrets
...
E_slope_pump  =  abs(beta_e(3)) * sig_u(3);    % pumpe 
...
% Stegresponsene viser hvordan istemperaturen påvirkes av hver inngang 
% over tid:
...
%  S_du bygges KUN for MV (ventil, pumpe). Halltemps bidrag til
...
% Summasjonsmatriser T_u:
Tu1 = buildCumSumMatrix(Np, Nc);
Tu2 = buildCumSumMatrix(Np, Nc);

% Normaliserer de fysiske grensene slik at de passer med den 
% normaliserte prosessmodellen:
...
% Leser inn bookingfilen og konverterer dato og klokkeslett til
% datetime-format:
...
% Lager full start- og sluttid for hver booking:
B.start_dt = B.dato + B.starttid;
B.slutt_dt = B.dato + B.sluttid;

% Filtrerer ut bookingene som gjelder 2-lane:
isLane2 = strcmpi(strtrim(string(B.lane)), '2-lane');
B2 = sortrows(B(isLane2, :), 'start_dt');

% Initialiserer referanse og temperaturgrenser med idle-verdier:
...
% Holder styr på forkjølingsperioder og terminalmål ved bookingstart:
mask_precool_all = false(N, 1);
terminal_ref_norm = NaN(N, 1);

for i = 1:height(B2)
    % Mapper brukergruppen til riktig temperaturintervall:
    ...
    % Elite og hobby får forkjøling før bookingstart:
    if lbl == "elite"
        leadtime = hours(3);
    elseif lbl == "hobby"
        leadtime = hours(3);
    ...
    % Beregner starttidspunktet for eventuell forkjøling:
    if leadtime > 0
        precool_start = B2.start_dt(i) - leadtime;
        % Forkjøling starter ikke før eventuell forrige booking er ferdig:
        for j = 1:height(B2)
            if B2.slutt_dt(j) > precool_start && B2.slutt_dt(j) <= B2.start_dt(i)
                precool_start = max(precool_start, B2.slutt_dt(j));
            end
        end
        % Bruker bookingens nedre temperaturgrense i forkjølingsperioden:
    ...
    % Setter referanse og temperaturgrenser under selve bookingen:
    ...
    % Lagrer terminalmål ved første tidssteg i bookingen:
...
% Normaliserer referanse og temperaturgrenser til samme skala som modellen:
r_norm    = (r_phys    - mu_y) ./ sig_y;
Tmin_norm = (Tmin_phys - mu_y) ./ sig_y;
Tmax_norm = (Tmax_phys - mu_y) ./ sig_y;

% Teller antall referanseendringer og terminalmål for kontroll:
n_transitions = sum(diff(r_phys) ~= 0);
n_terminals = sum(~isnan(terminal_ref_norm));
fprintf('  %d bookinger (2-lane), %d step-endringer, %d terminalmål\n\n', ...
    height(B2), n_transitions, n_terminals);

%% ================================================================
%  SEKSJON 8: PROSESSMODELL-SIMULATOR SETUP (AR(1)-støy)
%% ================================================================
fprintf('=== SEKSJON 8: Prosessmodell-simulator ===\n');

% Simulerer prosessmodellens istemperatur med de historiske inputsignalene:
t_all = (0:N-1)' * Ts;
y_model_all = lsim(sys_tf, u_sim_norm, t_all);

% Residualen brukes som grunnlag for det additive støyleddet:
residual_all = y_sim_norm - y_model_all;

% Sjekker residualens middelverdi for å vurdere om residualen har en 
% systematisk bias. Dersom middelverdien er nær null, kan AR(1)-støyen 
% modelleres uten konstantledd:
resid_mean = mean(residual_all, 'omitnan');
fprintf('  Residual: mean=%.4f (norm) = %.4f°C\n', ...
    resid_mean, resid_mean*sig_y);

% Standardavviket bestemmer støynivået, 
% og lag-1-korrelasjonen bestemmer minnet:
...
% Fast random seed gjør AR(1)-støyen repeterbar.
rng(42);
d_noise = zeros(N, 1);
for k = 2:N
...
%% ============================================================
%  SEKSJON 9: PRECOMPUTE KONSTANTE QP-MATRISER
%% ============================================================
...
% Kvadratisk sporingsledd for temperaturavviket.
H_q = W_quality * (Sdu' * Sdu);

% Kvadratisk glatthetsledd for ventil- og pumpeendringer.
H_m = blkdiag(R_du1 * eye(Nc), R_du2 * eye(Nc));

% Samler de kvadratiske QP-leddene, inkludert slack-straff.
H11 = 2*(H_q + H_m);
...
% Samler MV-grenser og myke temperaturgrenser i én ulikhetsmatrise.
...
% Grenser for ventilendringer, pumpeendringer og slack-variabel.
...
% Konverter til diskret tilstandsrommodell for prosessmodell-simulator:
...
% Halltemp (DV) i normalisert form — brukes både i prosess-sim og prediksjon:
hallTemp_norm = u_sim_norm(:, 2);

% Warm-start prosess-simulatoren med 3 inputs:
x_plant = zeros(nx, 1);
for kw = 1:N_warmup
...
for k = k_start:(N - Np - 1)
    ...
    % Filtrerer forrige prediksjonsfeil og bruker den som modellavvikskorreksjon.
    ...
    % Henter pris, referanse og temperaturgrenser over prediksjonshorisonten.
    ...
    % y_free: fri prediksjon før nye kontrollendringer legges til.
    % Inkluderer MV-historikk, halltemperaturens bidrag og estimert modellavvik:
    y_free = yk * ones(Np, 1);
    ...
    y_free = y_free + g_dv_h(1:Np) .* dhT_h;                 % fremtidig halltemp
    y_free = y_free + d_est;                                 % estimert modellavvik

    % Lineær QP-koeffisient fra det kvadratiske temperatursporingsleddet:
    E = y_free - r_h;
    f_q = W_quality * (Sdu' * E);
    ...
    % Finn bookingstarter i horisonten som skal få ekstra sporingsvekt:
    ...
    % Oppdaterer QP-grensene basert på nåværende MV-verdier og fri prediksjon:
    ...
    % Løser QP-en og finner optimal sekvens av fremtidige kontrollendringer:
    [z_opt, ~, exitflag] = quadprog(H_step, f_vec, A_full, b_upper, ...
        [], [], lb, ub, [], qpOpt);

    % Receding horizon: bare første ventil- og pumpeendring brukes:
    ...
    % Oppdaterer simulert istemperatur med valgt kontrolltrekk og AR(1)-støy:
    ...
    u2_phys_k = u2_mpc_n(k) * sig_u(3) + mu_u(3);    
...
%% ============================================================
%  SEKSJON 11: HISTORISK DRIFT SOM BASELINE
%  Også med 3 inputs (ventil, halltemp, pumpe).
%% ============================================================
fprintf('=== SEKSJON 11: Historisk baseline ===\n');

% Historisk baseline bruker de faktiske kontrollsignalene fra datasettet:
u1_base_phys_vec = u_sim_phys(:,1);   % historisk ventilsetpunkt
u2_base_phys_vec = u_sim_phys(:,3);   % historisk pumpehastighet
...
% Samme prosessmodell og støysekvens brukes for rettferdig sammenligning:
for k = k_start:N
...
u2_mpc_phys = u2_mpc_n * sig_u(3) + mu_u(3);  
...
% Evalueringsmasken inkluderer forkjøling i de totale KPI-ene:
valid = eval_mask & ~isnan(cost_mpc) & ~isnan(cost_base) & ...
        ~isnan(y_mpc_phys) & ~isnan(y_base_phys);

fprintf('  Forkjøling inkludert i totale KPI-er: %d steg (%.1f timer) av %d totalt\n', ...
...
fprintf('\n==============================================================\n');
fprintf('RESULTATER — Economic MPC — 2-LANE (3-input prosessmodell)\n');
fprintf('==============================================================\n');
...
fprintf('Prosessmodell-sim: 3-input håndtunet TF + AR(1) støy\n\n');

% Forkjøling skilles fra idle i gruppespesifikke KPI-er:
group_eval = group_all;
group_eval(mask_precool_all) = "precool";

% Gruppespesifikke KPI-er - bare de tre brukergruppene
fprintf('--- Gruppespesifikke KPI-er ---\n');
groups = {"elite", "hobby", "idle"};
for gi = 1:numel(groups)
    mask = group_eval == groups{gi} & valid;
    n_s = sum(mask);
...
% Tilleggsinformasjon — forkjøling separat (for dokumentasjon):
mask_pc = group_eval == "precool" & valid;
if sum(mask_pc) > 0
    n_h_pc = sum(mask_pc) * Ts / 3600;
    cost_pc_mpc = sum(cost_mpc(mask_pc));
    cost_pc_base = sum(cost_base(mask_pc));
    fprintf('\n  [Tilleggsinfo] Forkjøling (%.0fh): Cost MPC=%.0f, Hist=%.0f NOK\n', ...
        n_h_pc, cost_pc_mpc, cost_pc_base);
end
...
figure('Name','Istemperatur 2-lane','Position',[50 50 1400 550]);
hold on;
for k = 1:N-1
...
title(sprintf(['MPC 2-lane (3-input) | Cost: MPC=%.0f, Hist=%.0f NOK (%+.1f%%)\n' ...
...
plot(t_hr, u2_base_phys_vec, '--', 'Color', c_base, 'LineWidth', 1.2, ...
    'DisplayName', 'Historisk pumpehastighet');
plot(t_hr, u2_mpc_phys, '-', 'Color', c_pump, 'LineWidth', 1.5, ...
    'DisplayName', 'MPC pumpehastighet');
...
title('3-input prosessmodell → is'); legend('FontSize',10); grid on;
...
ylabel('°C'); title('Zoom: Istemperatur 2-lane'); grid on;
...
sgtitle('3-dagers zoom — MPC 2-lane (3-input prosessmodell)');
...
sgtitle(sprintf('KPI-er — MPC 2-lane (3-input) | Besparelse: %.1f NOK (%.1f%%)', ...
    savings, savings_pct));
...
%% ============================================================
 ...
 % Bygger DMC-matrisen som kobler fremtidige kontrollendringer 
 % til temperaturrespons:
...
% Bygger matrisen som summerer kontrollendringer til fremtidige MV-verdier:
...
% Beregner temperaturbidraget fra tidligere kontrollendringer:
